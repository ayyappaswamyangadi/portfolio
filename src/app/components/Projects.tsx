  ...
  return (
    ...
    >
      ...
      <div className="max-w-5xl mx-auto">
        ...
        {/* The grid above paginates client-side (2 per page), so only the first
            page is in the server-rendered HTML. This list puts every project —
            title, description, tech, live + GitHub links — in the initial
            markup so crawlers see all of them, not just page 1. */}
        <div className="sr-only">
          <h3>All projects</h3>
          <ul>
            {projects.map((p) => (
              <li key={p.id}>
                <a href={p.liveUrl}>{p.title}</a> — {p.description} Built with{" "}
                {p.tech.join(", ")}.
                {p.githubUrl && <a href={p.githubUrl}> Source code on GitHub</a>}
              </li>
            ))}
          </ul>
        </div>
